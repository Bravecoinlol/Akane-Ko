...
class slash(commands.Cog):
    ...
    def get_help_pages(self):
        ...
        embed.add_field(name="/setchannel", value="設定歡迎/離開頻道、管理頻道、身分組更新頻道（管理員限定）", inline=False)
        embed.add_field(name="/setroleupdatepm", value="設定是否啟用身分組變動時的私訊通知（管理員限定）", inline=False)
        ...
        embed.add_field(name="/createidentify", value="創建公開的身分組選擇面板（管理員限定）\n（如有設定，會自動發送公開/私訊通知）", inline=False)
        ...
        embed.add_field(name="/removeidentify", value="移除自己的身分組\n（如有設定，會自動發送公開/私訊通知）", inline=False)
        pages.append(embed)
